...
void game::logSDLError(ostream& cerr, const string &msg, bool fatal ){
...
void game::initSDL(SDL_Window* &window, SDL_Renderer* &renderer){
    if (SDL_Init(SDL_INIT_EVERYTHING) != 0){
        logSDLError(cerr, "SDL_Init", true);
    }
    window = SDL_CreateWindow(WINDOW_TITLE.c_str(), SDL_WINDOWPOS_CENTERED,
                          SDL_WINDOWPOS_CENTERED, SCREEN_WIDTH, SCREEN_HEIGHT, SDL_WINDOW_SHOWN);

    if (window == nullptr){ // kiem tra
        logSDLError(cerr, "CreateWindow", true);
    }
    renderer = SDL_CreateRenderer(window, -1, SDL_RENDERER_ACCELERATED | SDL_RENDERER_PRESENTVSYNC);

    if(renderer == nullptr){ // kiem tra
        logSDLError(cerr, "CreateRenderer", true);
    }
...
SDL_Texture* game::loadTexture(string path, SDL_Renderer *renderer) {
    SDL_Texture *newTexture = nullptr; // khoi tao texture rong
    SDL_Surface *loadedSurface = IMG_Load(path.c_str());
    if (loadedSurface == nullptr) {
        cerr << "Unable to load image " << path << " SDL image error: " << IMG_GetError() << endl;
    }
    else {
...
void game::backgroundAudio(const char* Song){
...
void game::effectAudio(const char* Song){
    Mix_Chunk* chunk = NULL;
    if (Mix_OpenAudio(44100, MIX_DEFAULT_FORMAT, 2, 2048) == -1){
 		cerr << Mix_GetError();
 	}
 	chunk = Mix_LoadWAV(Song);
 	if (chunk == NULL){
 		cerr << Mix_GetError();
 	}
 	if (!Mix_Playing(-1)){
